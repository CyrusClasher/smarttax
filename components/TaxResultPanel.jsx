"use client";
// components/TaxResultPanel.jsx
// Right-side results panel — regime comparison + slab breakdown.
// Receives the computed result object; derives winner/savings locally.
// Owns one piece of UI state: which regime's breakdown tab is active.
...
/**
 * @param {Object} result - Full result object from computeTax()
 */
export default function TaxResultPanel({ result }) {
  const [activeRegime, setActiveRegime] = useState("old");

  // Derive winner and savings from the result object
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 13, color: "#94a3b8" }}>
            You save{" "}
            <span style={{ color: "#4ade80", fontWeight: 700 }}>{fmt(savings)}</span>
      ...
      <div style={{ display: "flex", gap: 16 }}>
        <RegimeCard label="Old Regime" result={result.old} isWinner={winner === "old"} />
        <RegimeCard label="New Regime" result={result.new} isWinner={winner === "new"} />
      </div>
      ...
      <Card>
          ...
          {["old", "new"].map((r) => (
            <button
              ...
              style={{
                ...
                background: activeRegime === r ? "rgba(99,102,241,0.15)" : "transparent",
        ...
        >
          <span style={{ color: "#64748b" }}>Total Tax Payable (incl. 4% cess)</span>
